...
@click.option(
    "--report-yaml", "-r", type=str, help="Report.yaml file output from boomerang."
)
def from_reports(run_id: str, top_cliques: int, report_yaml: str):
    ...
    json_files = [
        x
        for x in listdir(boomer_output_dir)
        if (x.endswith(".json") and not x.startswith("singleton"))
    ]
    ...
    # ! This list below is empty.
    # ! The filenames of json/png are not the `id`s in boomerang's report output.
    # ! This needs to be sorted out before we connect everything.
    ...
    for id in top_clique_ids:
        ...
        # Add the image
        markdown_content.append(f"![Clique image](../{id}.png)")
